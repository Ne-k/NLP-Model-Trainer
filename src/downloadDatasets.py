...
with open('./datasets/mbpp.json', 'a') as f:
    f.write('[\n')
    for i, example in enumerate(mbpp['train']):
        input_text = example['text']
        target_text = example['code']
        data = {'input_text': input_text, 'target_text': target_text}
        json.dump(data, f, indent=4)
        if i != len(mbpp['train']) - 1:
            f.write(',\n')
    f.write('\n]')

mbppval = load_dataset('mbpp')

with open('./datasets/validate-mbpp.json', 'a') as f:
    f.write('[\n')
    for i, example in enumerate(mbppval['validation']):
        input_text = example['text']
        target_text = example['code']
        data = {'input_text': input_text, 'target_text': target_text}
        json.dump(data, f, indent=4)
        if i != len(mbppval['validation']) - 1:
            f.write(',\n')
    f.write('\n]')
#
